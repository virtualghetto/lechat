...
if($Q{action}[0]eq'setup'){
	...
	if($Q{message}[0]=~/^\s*$/){
		...
		del_all_messages() if $U{autokick}==3 and $U{status}<6;
...
sub send_alogin{
	print_start('admin');
	print qq|<center><$H{form}>|,hidden('action','setup'),qq|<table><tr><td align="left">$I{aloginname}</td><td><input type="text" name="nick" size="15"></td></tr><tr><td align="left">$I{aloginpass}</td><td><input type="password" name="pass" size="15"></td></tr><tr><td colspan="2" align="right">|,submit($I{aloginbut}),qq|</td></tr></table></form></center>|;
	print_end();
}
...
sub send_setup{
	...
	if($U{status}==9){
		...
		print thr(2),cfgts('sessionexpire'),cfgts('guestsexpire'),cfgts('messageexpire'),cfgts('kickpenalty'),cfgts('waitingexpire'),thr(2),cfgts('defaultrefresh'),cfgts('minrefresh'),cfgts('maxrefresh'),cfgts('floodlimit'),thr(2),
			cfgts('boxwidthdef'),cfgts('boxheightdef'),cfgts('maxmessage'),cfgts('maxname'),cfgts('minpass'),thr(2),cfgtm('title'),cfgtm('favicon'),cfgtm('noguests'),cfgtm('loginbutton'),thr(2),cfgta('header'),cfgta('footer'),thr(2),cfgta('rulestxt'),thr(2),cfgtb('nowchatting'),thr(2),cfgtb('entrymessage'),cfgtb('logoutmessage'),cfgtb('kickederror'),thr(2),
...
sub send_messages{
	get_waiting_count()if($T{guests}==3 and $U{status}>=7);
...
sub send_post{
	...
	print '</select></td></tr></table></form></td></tr><tr><td height="8"></td></tr><tr><td align="center"><table cellspacing="0"><tr><td>',frmpst('delete','last'),submit($I{butdellast},qq| style="$C{styledellast}"|),'</form></td><td>',frmpst('delete','all'),submit($I{butdelall},qq| style="$C{styledelall}"|),'</form></td><td width="10"></td><td>';
...
sub send_help{
	...
	print_start('rules');
	print "<h2>$I{rules}</h2>$C{rulestxt}<br><br><hr><h2>$I{help}</h2>",$U{status}>=1?"$I{helpguests}<br>":'',$U{status}>=2?"<br>$I{helpregs}<br>":'',$U{status}>=6?"<br>$I{helpmods}<br>":'',$U{status}>=7?"<br>$I{helpadmins}<br>":'',"<br><hr><center>$H{backtochat}<br>$H{versiontag}</center>";
	print_end();
}
...
sub send_controls{
	...
	print '<center><table cellspacing="0"><tr>';
...
sub send_login{
	...
	print_start('login');
	print qq|<center>$C{header}<$H{form} target="_parent">|,hidden('action','login'),qq|<table  $C{tableattributes}><tr title="$I{nickhelp}"><td align="left">$I{nickname}</td><td align="right"><input type="text" name="nick" size="15" style="$C{stylelogintext}"></td></tr><tr title="$I{passhelp}"><td align="left">$I{password}</td><td align="right"><input type="password" name="pass" size="15" style="$C{stylelogintext}"></td></tr>|;
	get_nowchatting();
	unless($T{noguests}){
...
sub create_session{
	...
	if($U{status}==1){# no known member, we have a guest:
		...
		create_waiting_session() if $T{guests}==3;# send guest to waiting room
	}
	write_new_session();
}
...
# returns cleaned up sessions and populates global variables
	...
	$T{noguests}=(!$T{guests} or $T{guests}>1 and !$T{admincount})?1:0;
	for($i=$#lines; $i>=0;$i--){
		%temp=sessionhash($lines[$i]);
...
sub create_waiting_session{
	...
	unless($inuse or $reentry or $T{noguests}){
		$U{status}=1;
		add_user_defaults();
	...
	send_error($I{errbadlogin}) if $inuse;
	send_error($I{errnoguests}) if $T{noguests};
...
sub save_config{
	...
	foreach(qw(header footer noguests rndguestcol loginbutton nowchatting csslogin stylelogintext stylecolselect styleenter tableattributes frameattributes framesizes))
	...
	foreach(qw(cssrules rulestxt entrymessage roomexit logoutmessage roomclean))
...
sub check_config{
	...
	$C{title}||='LE CHAT';
	foreach(qw(header footer noguests loginbutton rulestxt entrymessage logoutmessage kickederror roomentry roomexit regmessage kickedmessage roomclean nowchatting mesall mesmem mespm messtaff)){$C{$_}||=$I{"c$_"}}
...
sub set_config_defaults{
	...
	%C=();foreach(qw(
		...
		createlinks useextderef extderefurl allowmultiline allowfonts allowpms
		title favicon
		header footer noguests loginbutton rulestxt entrymessage logoutmessage kickederror roomentry roomexit regmessage kickedmessage roomclean nowchatting 
...
waitmessage=Welcome <NICK>, please be patient until an admin will let you into the chat room.<br><br>If this page doesn't refresh every <REFRESH> seconds, use the button below to reload it manually!
...
rules     =Rules
help      =Help
helpguests=All functions should be pretty much self-explaining, just use the buttons. In your profile you can adjust the refresh rate, font colour and your preferred input box size.<br><u>Note:</u> This is a chat, so if you don't keep talking, you will be automatically logged out after a while.
helpregs  =Members: You'll have some more options in your profile. You can adjust your font face, specify how long the entry page will be shown and you can change your password anytime.
helpmods  =Moderators: Notice the Admin-button at the bottom. It'll bring up a page where you can clean the room, kick chatters, view all active sessions and disable guest access completely if needed.
helpadmins=Admins: You'll be furthermore able to set all the guest access options, register guests, edit members and register new nicks without them beeing in the room.
stop_action=-help-
...
iQA/AwUBU6bV+cr7q1ZyCqQiEQJVqACfd8oLMErf6DklpFn9TsG05ClSbTsAoPe7
vT8III62ckC2c2B2oBRU7jlf
=Klsk
-----END PGP SIGNATURE-----
